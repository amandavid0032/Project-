...
class BookM
{
    ...
    public function getRecentBook(int $session)
    {
        ...
        $stmt = $this->_conn->prepare($sql);
        if ($stmt === false) {
            throw new \Exception('Statement preparation failed: ' . $this->_conn->error);
        }
    ...
    public function getMostPopularBook(int $session)
    {
        if ($session != 0) {
            ...
            $sql = "SELECT *, count(r.book_id) as bookc FROM books 
                ...
                ON r.book_id = books.id
                WHERE book_lang IN ($langIds) 
                GROUP BY books.id 
                ORDER BY count(*) DESC, books.id";
        } else {
            $sql = "SELECT *, count(r.book_id) as bookc FROM books 
                INNER JOIN (SELECT book_id FROM request) as r
                ON r.book_id = books.id  
    ...
    public function getMostUniqueGenre(int $session)
    {
        ...
        if ($session != 0) {
            ...
            $genreIds = implode(",", array_map('intval', $genre));
        ...
        if ($stmt === false) {
            ...
            return "Prepare failed: " . $this->_conn->error;
        }
    ...
    {
        $sql = "INSERT INTO wish_list (user_id,book_id) VALUES (? , ?)";
        $stmt = $this->_conn->prepare($sql);
    ...
    {
        $sql = "SELECT * FROM wish_list WHERE book_id = ? and user_id = ?";
