...
class RequestModel
{
    ...
    public function checkBookBelongsToOwner(int $bookId, int $bookOwner)
    {
        $checkBookBelongsToOwner = $this->conn->prepare("select * from books where id = ? and owner_id = ?");
        $checkBookBelongsToOwner->bind_param("ii", $bookId, $bookOwner);
        $checkBookBelongsToOwner->execute();
        $checkBookBelongsToOwnerRst = $checkBookBelongsToOwner->get_result();
        if ($checkBookBelongsToOwnerRst->num_rows > 0) {
            return true;
        } else {
            return false;
        }
    ...
    {
        $isBookAvailableStmt = $this->conn->prepare("select * from books where id = ?");
        $isBookAvailableStmt->bind_param("i", $bookId);
        $isBookAvailableStmt->execute();
        $isBookAvailableRst = $isBookAvailableStmt->get_result();
        $bookDetail = $isBookAvailableRst->fetch_assoc();
    ...
    public function rejectIssueRequest(int $requesterId, int $ownerId, int $bookId, string $cancelReason)
    {
        $date = Date("Y-m-d");
    ...
    public function acceptReturnRequest(int $requesterId, int $bookId, int $ownerId) : bool
    {
        ...
        $grntRqstQry = $this->conn->prepare("update request set status = ? where (requester_id = ? and book_id = ? and owner_id = ?)");
        $grntRqstQry->bind_param("iiii", $acceptReturnRequest, $requesterId, $bookId, $ownerId);
        $grntRqstQry->execute();
    ...
    {
        if ($type == 'returning') {
            $status = RequestModel::RETURNING_STATUS;
        } elseif ($type == 'returned') {
            $status = RequestModel::AVAILABLE_STATUS;
        }
