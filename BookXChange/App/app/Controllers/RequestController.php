...
class RequestController
{
    ...
    public function requestForBook(Request $request, Response $response)
    {
        ...
        if ($isRequest == 1) { //request to borrow the book
            ...
            $jsonMessage = [
                "isSuccess" => true,
                "message" => "This Book does't belongs to respected Owner."
            ];
        ...
        } elseif ($isRequest == 0) { //borrower cancelltion the book request by himself
            $isBookBelongsToOwner = $this->requestModelObj->checkBookBelongsToOwner($bookId, $bookOwner);
            $jsonMessage = [
                "isSuccess" => false,
                "message" => "This Book does't belongs to respected Owner."
    ...
    public function accept_reject_request(Request $request, Response $response)
    {
        ...
        if ($isAccept == 1 && $reason == null) {//to accept new book request
            ...
            if ($isBookAvailable) {
                ...
                $this->notificationObj->sendNoti($requesterId, $ownerId, $bookId, $acceptBookRequestByOwner);
                $jsonMessage = [
                    "isSuccess" => false,
        ...
        } elseif ($isAccept == 0 && $reason != null) {//to reject new book request
            $rejectIssueRequest = $this->requestModelObj->rejectIssueRequest($requesterId, $ownerId, $bookId, $reason);
            $this->notificationObj->sendNoti($requesterId, $ownerId, $bookId, $rejectBookRequestByOwner);
            $jsonMessage = [
                "isSuccess" => false,
                "message" => "Something went wrong."
            ];
        ...
        } elseif ($isAccept == 2 && $reason == null) {//to accept reutrn request.
            $acceptReturningBookRequest = $this->requestModelObj->acceptReturnRequest($requesterId, $bookId, $ownerId);
            $this->notificationObj->sendNoti($requesterId, $ownerId, $bookId, $acceptsReturnBookRequestByOwner);
            $jsonMessage = [
                "isSuccess" => false,
                "message" => "Already accepted return request."
            ];
            if($acceptReturningBookRequest) {
                $jsonMessage = [
                    "isSuccess" => true,
                    "message" => "Book return request accepted."
                ];
            }
        } else {
            $jsonMessage = [
